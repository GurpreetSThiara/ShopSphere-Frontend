...
const CartItem = ({ item, product, quantity }) => {
  ...
  const discountedPrice = useSelector(
    (state) => state.cart.totalDiscountedPrice
  );
  ...
  const handleDeleteFromCart = () => {
    dispatch(
      cartActions.deleteFromCart({ product: product, quantity: quantity })
    );
  };
      ...
      <Grid container>
        <Grid item xs={11}>
          <div className="inline-container">
            <img className="cart-image" alt="" src={product.imageUrl}></img>
